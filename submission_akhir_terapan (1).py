...
def anime_recommendations(input_anime, k=10):
    ...
    try:
        ...
        print(f"Anime dengan judul '{input_anime}' tidak ditemukan.")
        return []

    # Hitung skor kemiripan dengan semua anime lain
    ...
    # Ambil judul dan skor rekomendasi
    recommendations = [(df.iloc[i[0]].Title, i[1]) for i in sorted_scores]
...
input_anime = 'Naruto'
recommendations_with_scores = anime_recommendations(input_anime, k=10)

if recommendations_with_scores:
    # Extract the scores from the list of tuples
    scores = [score for title, score in recommendations_with_scores]

    # Calculate the average score
    average_similarity = sum(scores) / len(scores)

    print(f"Rata-rata skor kemiripan untuk rekomendasi '{input_anime}': {average_similarity:.4f}")

    print("\nAnime Rekomendasi:")
    for title, score in recommendations_with_scores:
        print(f"- {title} (Skor Kemiripan: {score:.4f})")
else:
    print(f"Tidak dapat menghitung rata-rata skor karena tidak ada rekomendasi ditemukan untuk '{input_anime}'.")

"""# **Evaluasi**

Disini masuk ke tahap evaluasi menggunakan metrik precision. Precision@K adalah metrik evaluasi yang digunakan untuk mengukur seberapa relevan hasil rekomendasi terhadap pengguna, dalam hal ini terhadap anime yang dijadikan input. Fungsi precision_at_k menerima parameter judul anime (input_anime) dan jumlah rekomendasi yang ingin dievaluasi (k). Fungsi ini pertama-tama memanggil anime_recommendations untuk mendapatkan daftar rekomendasi, kemudian mengambil deskripsi (About) dari anime input. Selanjutnya, dilakukan pemeriksaan apakah ada kemiripan kata atau genre dalam deskripsi rekomendasi dibandingkan dengan anime input, sebagai pendekatan untuk mengukur relevansi. Jika ditemukan kemiripan kata, rekomendasi tersebut dianggap relevan dan dihitung. Akhirnya, precision dihitung dengan membagi jumlah rekomendasi yang relevan dengan jumlah total rekomendasi (k), dan hasilnya ditampilkan dalam format persentase untuk menunjukkan seberapa akurat sistem rekomendasi bekerja.
"""

# Precision@K calculation
def precision_at_k(input_anime, k=10):
    recommendations = anime_recommendations(input_anime, k)
    if not recommendations:
        return 0.0

    # Ambil "About" dari anime input
    input_about = df[df['Title'] == input_anime]['About'].values[0]

    # Hitung jumlah rekomendasi yang "relevan"
    relevan_count = 0
    for title, _ in recommendations:
        rec_about = df[df['Title'] == title]['About'].values[0]
        # Cek apakah ada kesamaan genre atau kata kunci yang muncul (bisa kamu modifikasi)
        if any(token in rec_about for token in input_about.split()):
            relevan_count += 1

    precision = relevan_count / k
    print(f"Precision@{k} untuk '{input_anime}': {precision:.2f}")
    return precision

precision_at_k('Naruto', k=10)
...
# Ensure recommendations_with_scores is available and not empty
if 'recommendations_with_scores' in locals() and recommendations_with_scores:
    # Separate titles and scores for plotting
    titles = [title for title, score in recommendations_with_scores]
    scores = [score for title, score in recommendations_with_scores]

    # Create a bar plot of recommendations and their scores
    plt.figure(figsize=(10, 6))
    sns.barplot(x=scores, y=titles, palette='viridis')
    plt.title(f'Top 10 Rekomendasi Anime untuk "{input_anime}" berdasarkan Kemiripan Konten')
    plt.xlabel('Skor Kemiripan Kosinus')
    plt.ylabel('Judul Anime')
    plt.xlim(0, 1) # Cosine similarity is between 0 and 1
    plt.gca().invert_yaxis() # Invert y-axis to show the highest score at the top
    plt.show()
else:
    print("Tidak ada data rekomendasi untuk divisualisasikan. Jalankan kode rekomendasi terlebih dahulu.")